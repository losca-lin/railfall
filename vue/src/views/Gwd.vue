...
export default {
  ...
  methods: {
    ...
    save() {
      this.request.post("/gwd", this.form).then(res => {
        if (res.code === '200') {
    ...
    // 根据id删除一行数据
    del(id) {
      this.request.delete("/gwd/" + id).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/gwd/del/batch", ids).then(res => {
        if (res.code === '200') {
    ...
    exp() {
      window.open("http://localhost:9090/gwd/export")
    },
