...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    public UserDTO login(UserDTO userDTO) {
        ...
        if (one != null) {
            if (!one.getRole().equals(userDTO.getRole())){
                throw new ServiceException(Constants.CODE_600, "用户类型错误");
            }
    ...
    private User getUserInfo(UserDTO userDTO) {
        ...
        queryWrapper.eq("password", DigestUtil.md5Hex(userDTO.getPassword()));
        User one;
        try {
